import numpy as np 
import matplotlib.pyplot as plt 
import pyfits as pf
from scipy.interpolate import RectBivariateSpline as interp 
import Image
import ImageDraw
import pickle

def wfirstpupil(sz=1024):
	'''------------------------------------------------
	jwstpupil.py 

	code to generate a James Webb pupil mockup
	including the segment labelling!

	Owes a lot to Anthony Cheetham's IDL script
	------------------------------------------------'''

	#things to edit
	rprim = 2.4
	rsec = 0.35*rprim
	secpos = np.array([0.075*rprim,0])
	spiderthick = 0.025*rsec

	'''-------------------------------------------
	Initialise your arrays
	-------------------------------------------'''

	mask = np.ones((sz,sz))

	# get coordinates for the full un-cropped image
	xs, ys = np.linspace(-sz/2,sz/2,sz), np.linspace(-sz/2,sz/2,sz)
	xs *= 3*rprim/xs.max()
	ys *= 3*rprim/ys.max()

	xx,yy = np.meshgrid(xs,ys)
	rr = np.sqrt(xx**2 + yy**2)
	rr2 = np.sqrt((xx-secpos[0])**2 + (yy-secpos[1])**2)

	m2pix = sz/(xx.max()-xx.min())
	pix2m = 1./m2pix

	'''-------------------------------------------
	Put in the primary and secondary sizes
	-------------------------------------------'''

	mask /= mask.max()
	mask[rr>rprim] = 0
	mask[rr2<rsec] = 0

	'''-------------------------------------------
	Now do the spiders
	-------------------------------------------'''

	angles = [15., 105., 135., 225., 255., 345.]

	attaches = [5., 115., 125., 235., 245.,-5.]

	img = Image.fromarray(mask)
	draw = ImageDraw.Draw(img)

	for j, angle in enumerate(angles):
		
		angle *= np.pi/180.
		attach = attaches[j]*np.pi/180.

		start = 1.1*rprim*np.array([np.cos(angle),np.sin(angle)])-xx.min()
		stop = secpos+0.9*rsec*np.array([np.cos(attach),np.sin(attach)])-xx.min()

		normvec = np.array([start[1]-stop[1],stop[0]-start[0]])*spiderthick
		corners = np.round(m2pix*np.array([start+normvec,start-normvec,stop-normvec,stop+normvec]))

		#convert to pixels

		draw.polygon([tuple(p) for p in corners], fill=0)

	newmask = np.copy(np.asarray(img))


	'''-------------------------------------------
	Display
	-------------------------------------------'''

	# plt.figure(0)
	# plt.imshow(newmask,cmap=plt.cm.gray,extent=[xx.min(),xx.max(),xx.min(),xx.max()])
	# plt.xlabel('x (m)')
	# plt.ylabel('y (m)')
	# plt.title('WFIRST Pupil')
	# plt.draw()
	# plt.show()

	return newmask,xs,m2pix

def discrete_wfirst():
	'''Create a discrete pupil model for WFIRST'''

	nr     = 50              # rings within the pupil (should be ~> 50)
	rmax   = 2.4          # outer diameter
	# rmax   = 4.62 / 2.        # medium cross: 92 % of primary
	rmin   = 0.35*rmax#2.5/2.           # central obstruction: 50 % with Med-cross!
	# thick  = 0.257            # adopted spider thickness (meters)
	# thick = 0.4

	secpos = np.array([0.075*rmax,0])
	thick = 0.025*rmin*5

	srad = 0.1        # segment "radius"
	rad = np.sqrt(3)*srad # radius of the first hex ring in meters

	xs = np.array(())
	ys = np.array(())

	fig = plt.figure(0, figsize=(6,6))
	plt.clf()
	ax = plt.subplot(111)
	circ1 = plt.Circle((0,0), rmax, facecolor='none', linewidth=1)
	circ2 = plt.Circle((secpos[0],secpos[1]), rmin, facecolor='none', linewidth=1)
	ax.add_patch(circ1)
	ax.add_patch(circ2)
	#plt.clf()
	ax.axis([-rmax,rmax, -rmax,rmax], aspect='equal')

	# x = np.linspace(-rmax,rmax,25)
	# y = np.linspace(-rmax,rmax,25)

	# xs,ys = np.meshgrid(x,y)

	# xs, ys = xs.ravel(),ys.ravel()

	for i in range(1-nr, nr, 1):
	    for j in xrange(1-nr, nr, 1):
	        x = srad * (i + 0.5 * j)
	        y = j * np.sqrt(3)/2.*srad
	        if (abs(i+j) < nr):
	            xs = np.append(xs, x)
	            ys = np.append(ys, y)

	# modifications to match the actual telescope pupil (1): diameter constraints
	# -----------------------------------------------------------------------
	xx, yy = xs.copy(), ys.copy()        # temporary copies
	xs, ys = np.array(()), np.array(())  # start from scratch again

	osize = 0.01 * 2*rmax # oversize is a fixed % of total telescope size

	for i in range(xx.size):
	    thisrad = np.sqrt(xx[i]**2 + yy[i]**2)
	    thisrad_off =  np.sqrt((xx[i]-secpos[0])**2 + (yy[i]-secpos[1])**2)
	    if thisrad < (rmax-osize) and (thisrad_off > (rmin+osize)):# + 0.1*srad)):
		        xs = np.append(xs, xx[i])
		        ys = np.append(ys, yy[i])

	# modifications to match the actual telescope pupil (2): spiders
	# -----------------------------------------------------------
	rm_spiders = True
	spiderangles = np.pi/180.*np.array([15., 105., 135., 225., 255., 345.])
	attaches = np.pi/180.*np.array([5., 115., 125., 235., 245.,-5.])
	# thicks = rmin*np.array([0.2,0.6,0.2])
	spiderindices = []
	sides = ['top','top','bottom','bottom','right','right']

	if rm_spiders:
		xx, yy = xs.copy(), ys.copy()        # temporary copies
		xs, ys = np.array(()), np.array(())  # start from scratch again

		for j,spider in enumerate(spiderangles):

			attach = attaches[j]

			start = 1.2*rmax*np.array([np.cos(spider),np.sin(spider)])
			stop = secpos+0.6*rmin*np.array([np.cos(attach),np.sin(attach)])
			xmin, xmax = np.min([start[0],stop[0]]), np.max([start[0],stop[0]])
			ymin, ymax = np.min([start[1],stop[1]]), np.max([start[1],stop[1]])

			for i in range(xx.size):

				# if sides[j]=='left':
				# 	condition = yy[i]>=0
				# elif sides[j]=='top':
				# 	condition = xx[i]>=0
				# elif sides[j]=='bottom':
				# 	condition = yy[i]<=0

				xin = (xmin <= xx[i] <= xmax)
				yin = (ymin <= yy[i] <= ymax)

				condition = (xin and yin)

				grad = (stop[1]-start[1])/(stop[0]-start[0])
				dist = abs(grad*(xx[i]-secpos[0])-(yy[i]-secpos[1]))/np.sqrt(1+grad**2)

				if dist<thick and condition==True:
					spiderindices.append(i)

		xs = np.delete(xx,spiderindices)
		ys = np.delete(yy,spiderindices)  

	ns = xs.size # number of samples

	# final modification: global pupil rotation
	# -----------------------------------------
	th0 = 0#-45.0 * np.pi / 180.0
	rmat = np.matrix([[np.cos(th0), np.sin(th0)], [np.sin(th0), -np.cos(th0)]])

	coords = np.array([xs, -ys])
	coords2 = np.array(np.dot(rmat, coords))

	xs = coords2[0,:]#np.reshape(coords2[0], ns)
	ys = coords2[1,:]#np.reshape(coords2[1], ns)



	# plot segments
	# -------------
	r0 = srad/np.sqrt(3)
	th = 2*np.pi*np.arange(6)/6. + np.pi/6.

	for i in range(xs.size):
		hx = xs[i] + r0 * np.cos(th + th0)
		hy = ys[i] + r0 * np.sin(th + th0)
		ax.fill(hx, hy, fc='none', linewidth=1)

	ax.plot(xs, ys, 'r.')

	np.savetxt("../swiftmask.txt", np.transpose((xs,ys)), 
	           fmt='%12.9f')

	print "--------------------------------------------------"
	print "%d pupil sample points were included in the pupil " % xs.size
	print "--------------------------------------------------"

	plt.show()

	mask = np.array([xs,ys])
	mask = mask.T

	data = {'mask'   : mask
	        }

	# myf = open('./geometry/wfirstmask.pick','w')
	# pickle.dump(data, myf, -1)
	# myf.close()
	np.savetxt('./geometry/wfirstmask.txt',mask)

	print 'Mask coordinates successfully saved'

